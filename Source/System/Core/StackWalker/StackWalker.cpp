...
BOOL StackWalker::ShowCallstack(
	...
	LPVOID pUserData,
	char *	pDestBuffer,// буффер в который запишется текст call stack
	int		nMaxSize )	// максимальная длинна текста (включая null-term)
{
  CONTEXT c;
...
#error "Platform not supported!"
  ...
  for (frameNum = 0; ; ++frameNum )
  {
	...
	if ( firstEntry != et ) // skip call in StackWalker
	{
		int nSize = CallstackEntryToString( csEntry, pDestBuffer, nMaxSize );
...
*
*/
int StackWalker::CallstackEntryToString(
	const CallstackEntry &	entry,		// ссылка на запись
	char *					pDestBuffer,// буффер в который запишется текст call stack
	uint					nMaxSize )	// максимальная длинна текста (включая null-term)
{
	if ( ( NULL != pDestBuffer ) && ( nMaxSize > 0 ) )
	{
		CStr entry_str;

		entry_str.Add( "\t" );
		entry_str.Add( entry.name );
		entry_str.Add( " ( " );
		entry_str.Add( entry.lineFileName );
		entry_str.Add( " line:" );
		entry_str.AddNumber( entry.lineNumber );
		entry_str.Add( " )\n" );
		
		uint nSize = entry_str.GetSize();

		if ( nMaxSize >= nSize )
		{
			memcpy( pDestBuffer, entry_str.GetString(), nSize );
			return nSize;
		}
	}
